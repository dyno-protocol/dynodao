...
interface ISEAERC20 is IERC20Mintable, IERC20MetadataUpgradeable {
...
contract SeaTreasury is OwnableUpgradeable {
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20Upgradeable( Wave ).balanceOf( msg.sender ); // Can only borrow against Wave held
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        ISEAERC20( SEA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( ISEAERC20( SEA ).totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** ISEAERC20( SEA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
