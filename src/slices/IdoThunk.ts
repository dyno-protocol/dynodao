...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const signer = provider.getSigner();
    const idoContract = new ethers.Contract(addresses[networkID].USDT_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
    try {
    ...
    // const stakeAllowance = await idoContract.allowance(address, addresses[networkID].USDT_ADDRESS);
    ...
    const busdContract = new ethers.Contract(addresses[networkID].USDT_ADDRESS as string, ierc20Abi, provider);
