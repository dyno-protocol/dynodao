...
export const calcBondDetails = createAsyncThunk(
  ...
  async (
    ...
    const debtRatio =
      (await bondContract.standardizedDebtRatio()) / Math.pow(10, 9);
    ...
    if (
      !!value &&
      parseFloat(bondQuote.toString()) > maxBondPrice / Math.pow(10, 9)
    ) {
      const errorString =
...
export const bondAsset = createAsyncThunk(
  ...
  async (
    ...
    console.log("alex: Bond value:", Number(valueInWei));
    console.log("alex: Bond max:", maxPremium);
    console.log("alex: Bond add:", depositorAddress);
    try {
      bondTx = await bondContract.depositEx(
        valueInWei,
        maxPremium,
        depositorAddress
      );
  ...
  async (
    ...
    try {
      ...
      const pendingTxnType =
        "redeem_bond_" + bond + (autostake === true ? "_autostake" : "");
