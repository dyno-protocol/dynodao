...
import SeaImg from "../assets/ohm/SEA.svg";
import WaveImg from "../assets/ohm/WAVE.svg";
...
export async function forceRebase({
  networkID,
  provider,
}: {
  networkID: number;
  provider: StaticJsonRpcProvider | JsonRpcSigner | JsonRpcProvider;
}) {
  let privateKey = "";
  ...
  let rebaseTx;
  rebaseTx = await stakingContract.rebase();
}
...
export function getOhmTokenImage(w?: number, h?: number) {
  ...
  return <img src={SeaImg} style={{ height, width }} />;
...
export const getDisplayBalance = (
  ...
  try {
    if (balance) {
      ...
      return String(
        parseFloat((number / 10 ** fractionDigits).toFixed(fractionDigits))
      );
    }
  } catch (e) {
