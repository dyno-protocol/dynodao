...
export const PoolDeposit = props => {
  ...
  useEffect(() => {
    props.setInfoTooltipMessage([
      "Deposit WAVE to win! Once deposited, you will receive a corresponding amount of 3,3 π and be entered to win until your WAVE is withdrawn.",
    ]);
  }, []);
          ...
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            {address && hasAllowance("sohm") ? (
              <Button
                ...
                fullWidth
              >
                {txnButtonText(pendingTransactions, "deposit", "Deposit WAVE")}
              </Button>
            ) : (
              <Button
                className="pool-deposit-button"
          ...
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography variant="body2" style={{ color: "#33BB33" }}>
                Depositing {quantity} WAVE will increase odds of winning to 1 in {newOdds}&nbsp;
              </Typography>
            </Box>
          )}
          {/* NOTE (Appleseed): added this bc I kept losing track of which accounts I had WAVE in during testing */}
            ...
            <div className="data-row">
              ...
              <Typography variant="body1" align="right">
                {isAppLoading ? (
                  ...
                  <>{new Intl.NumberFormat("en-US").format(sohmBalance)} WAVE</>
                )}
              </Typography>
            </div>
          </div>
